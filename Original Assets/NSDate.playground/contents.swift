...
extension NSDate {
    ...
    func haveDaysElapsedIngoringTime(days: Int) -> Bool {
        ...
        let diff = NSDate.difference(fromDate: NSCalendar.currentCalendar().dateFromComponents(todayComps)!, toDate: NSCalendar.currentCalendar().dateFromComponents(thisComps)!)
        print(diff.day)
        return diff.day >= days
    }
...
let x = NSDate(day: 23, hour: 23)

x.haveDaysElapsedIngoringTime(2)
